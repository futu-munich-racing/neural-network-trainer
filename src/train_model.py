...
def _parse_fn(example_serialized, is_training=False):
    ...
    image = tf.io.decode_jpeg(parsed['image']) #tf_tools.decode_jpeg(parsed['image'])
...
def main(argv):
    ...
    raw_trainset = tf.data.TFRecordDataset(os.path.join(args.train_dir, 'train.tfrecord'))
    parsed_trainset = raw_trainset.map(_parse_fn)

    # Read validation dataset
    raw_validationset = tf.data.TFRecordDataset(os.path.join(args.val_dir, 'val.tfrecord'))
    ...
    save_best = ModelCheckpoint(args.output_model_file,
                                monitor='val_loss',
